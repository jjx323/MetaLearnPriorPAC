...
class Domain2D(Domain):
    ...
    '''
    def __init__(self, comm=MPI.COMM_SELF, low_point=[0, 0], high_point=[1, 1], nx=100, ny=100, mesh_type='P', mesh_order=2):
        super().__init__(mesh_type, mesh_order)
        self.low_point, self.high_point = low_point, high_point 
        self.nx, self.ny = nx, ny
        self._mesh = dl.RectangleMesh(dl.Point(low_point[0], low_point[1]), \
                                     dl.Point(high_point[0], high_point[1]), nx, ny)
        self._function_space = dl.FunctionSpace(self._mesh, self.mesh_type, self.mesh_order)
    ...
    def update(self, low_point=[0, 0], high_point=[1, 1], nx=100, ny=100, mesh_type='P', mesh_order=2):
        ...
        self._mesh = dl.RectangleMesh(dl.Point(low_point[0], low_point[1]), \
                                     dl.Point(high_point[0], high_point[1]), nx, ny)
        self._function_space = dl.FunctionSpace(self._mesh, mesh_type, mesh_order)
