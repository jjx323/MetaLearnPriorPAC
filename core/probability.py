...
class GaussianFiniteRank(object):
    ...
    def __init__(self, domain, domain_=None, mean=None, num_KL=None, 
                 alpha=1.0, beta=1.0, s=2, sieve=False):
        if domain_ is None:
            domain_ = domain
        self.domain = domain
        self.domain_ = domain_
        self.Vh = domain.function_space
    ...
    def set_eigensystem(self, eigval=None, eigvec=None):
        if (eigval is None) and (eigvec is None):
            raise ValueError("Please provide eigval and eigvec")
    ...
    def generate_sample_zero_mean(self, num_sample=1):
        ...
        ## Sieve prior, random choice the eigenfunctions
        if self.sieve == True:
            vector = np.random.choice([0, 1], size=len(val), p=[0.5, 0.5])
            val = vector*val
