...
if comput_learned_fS == True:
    ...
    np.save(meta_data_dir + env + "_learned_error_list", np.array(learned_error_list, dtype=np.float64))
    # np.save(meta_results_dir + "estimates_learn_MAP", (estimates_learn, final_error_learn))

unlearned_error_list = np.load(meta_results_dir + env + "_unlearned_error_list.npy", allow_pickle=True)
learned_error_list_f = np.load(meta_results_dir + env + "_learned_error_list_f.npy", allow_pickle=True)
learned_error_list_fS = np.load(meta_data_dir + env + "_learned_error_list.npy", allow_pickle=True)

error_file_names = results_fig_table + env + "_errors_" + ".txt"
...
for idx in [1, 5, 10, 15, 20, 25, 30, 35]:
    ...
    unlearned_error_std= np.std(np.array(unlearned_error_list)[:, idx])
    learned_error_std_f = np.std(np.array(learned_error_list_f)[:, idx])
    learned_error_std_fS = np.std(np.array(learned_error_list_fS)[:, idx])
    ...
    with open(error_file_names, "a") as f:
        formatted_string = f"iter_num = {idx:2d}, unlearned = {unlearned_error_avg:.4f} +- {unlearned_error_std:.4f}, "
        formatted_string += f"learned_f = {learned_error_avg_f:.4f} +- {learned_error_std_f:.4f}, "
        formatted_string += f"learned_fS_L2 = {learned_error_avg_fS:.4f} +- {learned_error_std_fS:.4f}"
        formatted_string += "\n"
        f.write(formatted_string)

## -------------------------------------------------------------------------------------------------------------------
if env == "complex":
    error_file_names = results_fig_table + env + "_errors_odd_" + ".txt"
    file_process(error_file_names)

    for idx in [1, 5, 10, 15, 20, 25, 30, 35]:
        unlearned_error_avg = np.mean(np.array(unlearned_error_list)[1::2, idx])
        learned_error_avg_f = np.mean(np.array(learned_error_list_f)[1::2, idx])
        learned_error_avg_fS = np.mean(np.array(learned_error_list_fS)[1::2, idx])

        unlearned_error_std= np.std(np.array(unlearned_error_list)[1::2, idx])
        learned_error_std_f = np.std(np.array(learned_error_list_f)[1::2, idx])
        learned_error_std_fS = np.std(np.array(learned_error_list_fS)[1::2, idx])
...
## -------------------------------------------------------------------------------------------------------------------
if env == "complex":
    error_file_names = results_fig_table + env + "_errors_even_" + ".txt"
    file_process(error_file_names)

    for idx in [1, 5, 10, 15, 20, 25, 30, 35]:
        unlearned_error_avg = np.mean(np.array(unlearned_error_list)[::2, idx])
        learned_error_avg_f = np.mean(np.array(learned_error_list_f)[::2, idx])
        learned_error_avg_fS = np.mean(np.array(learned_error_list_fS)[::2, idx])

        unlearned_error_std= np.std(np.array(unlearned_error_list)[::2, idx])
        learned_error_std_f = np.std(np.array(learned_error_list_f)[::2, idx])
        learned_error_std_fS = np.std(np.array(learned_error_list_fS)[::2, idx])
