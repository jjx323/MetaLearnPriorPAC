...
from Mix_Gaussian import Mix_Gauss_IS_proposal, pCN_preserve_GMM


"""
In the following, we compare the performance by SMC with mixture Gaussian approximation for different prior measures.
"""
...
parser.add_argument('--num_particles', type=int, default=1000, help='total number of particles')
...
"""
Sampling by using SMC with mixture Gaussian approximation for test examples with unlearned prior measures 
"""

prior_unlearn = GaussianFiniteRank(
    domain=domain, domain_=domain, alpha=0.1, beta=10, s=2, sieve=sieve
    )
...
if comput_unlearned == True:
    for idx in indexs:
        ...
        while 1:
            ...
            # print("Potential function computed.")
            h = smc.search_h(h=h0) 
            h_list.append(h)
            sum_h += h
            print("h = %4.15f" % h)
            ...
            gmm = Mix_Gauss_IS_proposal(model.prior)
            print("project dim = ", gmm.project_dim)
            gmm.update_samples(smc.particles)
            sampler = pCN_preserve_GMM(model, gmm, reduce_chain=np.int64(1e4))
            smc.transition(sampler=sampler, len_chain=np.int64(5), info_acc_rate=False)
            ...
            # smc.transition(sampler=sampler, len_chain=np.int64(10), info_acc_rate=False)
...
"""
Sampling by using SMC with mixture Gaussian approximation for test examples with learned prior measures f(\theta)
"""
...
"""
Sampling by using SMC with mixture Gaussian approximation for test examples with learned prior measures f(S;\theta)
"""
...
tmp = env + "_smc_mix_computation_time.txt"
