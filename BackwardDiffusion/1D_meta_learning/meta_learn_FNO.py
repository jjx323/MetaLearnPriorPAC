...
alpha_hyper_prior, beta_hyper_prior = 0.001, 0.007
...
hidden_dim = 15
np.save(meta_results_dir + "hidden_dim", hidden_dim)
nnprior_mean = FNO1d(
...
for kk in range(max_iter):
    ...
    if kk % 10 == 0:
        ...
        error_train_list = []
        for idx_train in analysis_list:
            preddicted_mean = nnprior_mean_boundary(Sy_train_list[idx_train], gridx_tensor)
            preddicted_mean = preddicted_mean.detach().numpy()
            error_train_list.append(np.linalg.norm(preddicted_mean - u_train_list[idx_train])/np.linalg.norm(u_train_list[idx_train]))
        tmp = np.mean(error_train_list)
        print("average test error (train): %.5f" % tmp, end="; ")
        training_error_list.append(tmp)

        error_test_list = []
        for idx_test in analysis_list:
            preddicted_mean = nnprior_mean_boundary(Sy_test_list[idx_test], gridx_tensor)
            preddicted_mean = preddicted_mean.detach().numpy()
            error_test_list.append(np.linalg.norm(preddicted_mean - u_test_list[idx_test])/np.linalg.norm(u_test_list[idx_test]))
        tmp = np.mean(error_test_list)
        print("average test error (test): %.5f" % tmp)
        testing_error_list.append(tmp)
        ## --------------------------------------------------------
        fun = fe.Function(domain.function_space)
        equ_solver = EquSolver(domain_equ=domain, T=T, num_steps=num_steps, \
                                points=np.array([meta_data_x_test[0]]).T, m=None)
        targets = torch.tensor(meta_data_y_test[0], dtype=torch.float32)
        Sy = d2fun(meta_data_y_test[0], equ_solver)
        prior.mean_vec_learn = nnprior_mean_boundary(Sy, gridx_tensor)
        fun.vector()[:] = np.array(prior.mean_vec_learn.detach().numpy())
        equ_solver = EquSolver(domain_equ=domain, T=T, num_steps=num_steps, \
                                points=np.array([meta_data_x_test[0]]).T, m=fun)
        ...
        if env == "simple":
            min_val, max_val = eval_min_max([
                u_meta_fun_test.vector()[:], sol_fun_test.vector()[:], fun.vector()[:],
                sol_fun_est.vector()[:]
                ])
            fe.plot(u_meta_fun_test, label="one_meta_u")
            fe.plot(sol_fun_test, label="sol_meta_u")
            ...
            # plt.close()
            if learn_var == True:
                print("learned_gamma: ", torch.exp(prior.log_gamma_learn).item(), end="; ")
                print("learned_log_gamma: ", prior.log_gamma_learn.item())
        elif env == "complex":
            plt.figure(figsize=(13, 5))
            plt.subplot(1,2,1)
            min_val, max_val = eval_min_max([
                u_meta_fun1_test.vector()[:], sol_fun1_test.vector()[:], fun.vector()[:],
                sol_fun_est.vector()[:]
                ])
            fe.plot(u_meta_fun1_test, label="one_meta_u")
            fe.plot(sol_fun1_test, label="sol_meta_u")
            fe.plot(fun, label="estimated_mean")
            fe.plot(sol_fun_est, label="sol_estimated_meta")
            plt.ylim([min_val-0.1, max_val+0.1])
            plt.legend()
            ...
            fun = fe.Function(domain.function_space)
            equ_solver = EquSolver(domain_equ=domain, T=T, num_steps=num_steps, \
                                    points=np.array([meta_data_x_test[1]]).T, m=None)
            targets = torch.tensor(meta_data_y_test[1], dtype=torch.float32)
            Sy = d2fun(meta_data_y_test[1], equ_solver)
            prior.mean_vec_learn = nnprior_mean_boundary(Sy, gridx_tensor)
            fun.vector()[:] = np.array(prior.mean_vec_learn.detach().numpy())
            equ_solver = EquSolver(domain_equ=domain, T=T, num_steps=num_steps, \
                                    points=np.array([meta_data_x_test[1]]).T, m=fun)
            sol_est = equ_solver.forward_solver()
            sol_fun_est = fe.Function(domain.function_space)
            sol_fun_est.vector()[:] = np.array(sol_est)
            
            min_val, max_val = eval_min_max([
                u_meta_fun2_test.vector()[:], sol_fun2_test.vector()[:], fun.vector()[:],
                sol_fun_est.vector()[:]
                ])
            fe.plot(u_meta_fun2_test, label="one_meta_u")
            fe.plot(sol_fun2_test, label="sol_meta_u")
            fe.plot(fun, label="estimated_mean")
            fe.plot(sol_fun_est, label="sol_estimated_meta")
            plt.ylim([min_val-0.1, max_val+0.1])
            plt.legend()
...
if learn_var == True:
    np.save(
        meta_results_dir + env + str(equ_nx) + "_meta_FNO_learned_var_prior",
        np.array(learned_var_list)
    )
    plt.figure()
    plt.plot(np.array(learned_var_list), label="learned_var")
    plt.legend()
    plt.savefig(meta_results_dir + env + str(equ_nx) + "_meta_FNO_learned_var_prior.png")
    plt.close()
...
np.save(meta_results_dir + env + str(equ_nx) + "_FNO_training_error_list", training_error_list)
np.save(meta_results_dir + env + str(equ_nx) + "_FNO_testing_error_list", testing_error_list)
