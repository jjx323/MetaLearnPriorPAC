...
error_file_names = results_fig_table + "errors_complex.txt"
file_process(error_file_names)
times_file_names = results_fig_table + "complex_record_time_map.txt"
...
for idx in sample_draw_list:
    if idx % 2 == 0:
        ...
        fe.plot(tmp_fun)
plt.ylim([-12, 11])
plt.title("(a) Environment samples (Branch 1)", pad=15)  # 增加pad参数
...
fe.plot(tmp_fun, label=r"Learned mean $f_m(\theta_1)$",
        color="blue", linestyle="--")
tmp_fun.vector()[:] = np.array(save_learned_mean_fS[i1])
fe.plot(tmp_fun, label=r"Learned mean $f_m(S;\theta_1)$",
        color="red", linestyle="-.")
plt.legend(loc='upper left')
plt.ylim([-10.5, 8])
plt.title("(b) Estimated prior means (Branch 1)", pad=15)
...
plt.legend(loc='upper left')
plt.ylim([-0.5, 5])
plt.title("(c) Estimated prior means (Branch 1)", pad=15)
...
for idx in sample_draw_list:
    ...
    fe.plot(tmp_fun)
plt.ylim([-11, 12])
plt.title("(d) Environment samples (Branch 2)", pad=15)
...
plt.title("(e) Estimated prior means (Branch 2)", pad=15)
...
fe.plot(tmp_fun, label=r"Learned mean $f_m(S;\theta_1)$",
        color="red", linestyle="-.")
plt.legend(loc='lower left')
plt.title("(f) Estimated prior means (Branch 2)", pad=15)
plt.ylim([-7, 5.5])

plt.tight_layout(pad=0.8, w_pad=2, h_pad=3)
plt.savefig(results_fig_table + "ExampleComplex1.jpg", dpi=1000)
plt.savefig(results_fig_table + "ExampleComplex1.pdf")
plt.close("all")
