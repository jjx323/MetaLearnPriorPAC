...
This piece of code is used to see the effect of different hidden dimensions of the FNO
"""
...
parser.add_argument('--hidden_dims', type=int, nargs='+', default=[5, 10, 15, 20, 25, 30],
                    help='List of hidden dimensions for the neural network model')
...
for hidden_dim in hidden_dims:
    ...
    if learn_var == True:
        np.save(
            meta_results_dir + env + str(equ_nx) + "_" + str(hidden_dim) + "_" + "_meta_FNO_learned_var_prior",
            np.array(learned_var_list)
        )
        plt.figure()
        plt.plot(np.array(learned_var_list), label="learned_var")
        plt.legend()
        plt.savefig(meta_results_dir + env + str(equ_nx) + "_" + str(hidden_dim) + "_" + "_meta_FNO_learned_var_prior.png")
        plt.close()
        
    if with_hyper_prior == True:
        torch.save(
            nnprior_mean.state_dict(), meta_results_dir + env +
            str(equ_nx) + "_" + str(hidden_dim) + "_" + "_meta_FNO_mean_prior"
        )
        np.save(meta_results_dir + env + str(equ_nx) + "_" + str(hidden_dim) + "_" +
                "_meta_FNO_loss_prior", loss_list)
        np.save(
            meta_results_dir + env + str(equ_nx) + "_" + str(hidden_dim) + "_" + "_meta_FNO_log_gamma_prior",
            prior.log_gamma_learn.detach().numpy()
        )
    else:
        torch.save(
            nnprior_mean.state_dict(), meta_results_dir + env + "_" + str(hidden_dim) + "_" +
            str(equ_nx) + "_meta_FNO_mean"
        )
        np.save(meta_results_dir + env + str(equ_nx) + "_" + str(hidden_dim) + "_" + "_meta_FNO_loss", loss_list)
        np.save(
            meta_results_dir + env + str(equ_nx) + "_" + str(hidden_dim) + "_" + "_meta_FNO_log_gamma",
            prior.log_gamma_learn.detach().numpy()
        )
